...
interface MessagesDao {
    ...
    fun getMessagesSorted(
        conversationSid: String
    ): DataSource.Factory<Int, MessageDataItem>

    // Get all Messages sorted
    @Query("SELECT * FROM message_table WHERE conversationSid = :conversationSid AND `index`> :messageIndex ORDER BY CASE WHEN `index` < 0 THEN dateCreated ELSE `index` END ASC")
    fun getMessagesFromIndex(
        conversationSid: String,
        messageIndex: Long
    ): DataSource.Factory<Int, MessageDataItem>
    ...
    @Query("UPDATE message_table SET messagesCount = :messagesCount WHERE conversationSid = :conversationSid")
    fun updateMessagesCount(conversationSid: String, messagesCount: Long)
