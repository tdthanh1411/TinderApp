...
inline fun Snackbar.onDismissed(crossinline block: (event: Int) -> Unit) =
    addCallback(OnSnackbarDismissed(block))
...
fun String.extractUrl(): List<String> {
    ...
    val regexString = "\\b(https://|www[.])[A-Za-z0-9+&@#/%?=~_()|!:,.;]*[-A-Za-z0-9+&@#/%=~_()|]"
    val pattern = Pattern.compile(regexString, Pattern.CASE_INSENSITIVE)
    val matcher = pattern.matcher(this)
    while (matcher.find()) {
        list.add(this.substring(matcher.start(0), matcher.end(0)))
    }
    if (list.size == 0) {
        return emptyList()
    }
