...
import com.twilio.conversation.common.asParticipantDataItem
import com.twilio.conversation.common.asToParticipantsData
...
class ConversationsException(val error: ConversationsError, val errorInfo: ErrorInfo? = null) :
    Exception("$error") {
    constructor(errorInfo: ErrorInfo) : this(ConversationsError.fromErrorInfo(errorInfo), errorInfo)
}

suspend fun createAndSyncClient(
    context: Context,
    token: String,
    properties: Properties = Properties.newBuilder().createProperties()
): ConversationsClient {
...
private suspend fun createConversationsClient(
        ...
        ConversationsClient.create(
            ...
            object : CallbackListener<ConversationsClient> {
                override fun onSuccess(conversationsClient: ConversationsClient) =
                    continuation.resume(conversationsClient)

                override fun onError(errorInfo: ErrorInfo) {
                    continuation.resumeWithException(ConversationsException(errorInfo))
                }
            })
    }

private suspend fun ConversationsClient.waitForSynchronization(): Unit =
...
suspend fun ConversationsClient.registerFCMToken(token: String) =
...
suspend fun ConversationsClient.unregisterFCMToken(token: String) =
    suspendCancellableCoroutine<Unit> { continuation ->
...
suspend fun ConversationsClient.updateToken(token: String) =
    suspendCancellableCoroutine<Unit> { continuation ->
...
suspend fun ConversationsClient.createConversation(friendlyName: String): Conversation =
            ...
            synchronized<Unit>(complete) {
                ...
                if (conversation.synchronizationStatus == Conversation.SynchronizationStatus.FAILED) {
                    val errorInfo = ErrorInfo(
                        CONVERSATION_NOT_SYNCHRONIZED,
                        "Conversation synchronization failed: ${conversation.sid}}"
                    )
...
suspend fun Conversation.removeParticipant(participant: Participant): Unit =
    suspendCancellableCoroutine { continuation ->
...
suspend fun Conversation.addParticipantByIdentity(
    identity: String,
...
suspend fun Conversation.addParticipantByAddress(
    address: String,
    proxyAddress: String,
    attributes: Attributes = Attributes.DEFAULT
): Unit = suspendCancellableCoroutine { continuation ->
...
suspend fun Conversation.getListParticipant(): List<ParticipantDataItem> =
    suspendCancellableCoroutine { continuation ->
        continuation.resume(asToParticipantsData(participantsList))
    }

suspend fun Conversation.getParticipantName(credentialStorage: CredentialStorage): String =
    suspendCancellableCoroutine { continuation ->
        val identityName = when (participantsList.size) {
            0, 1 -> {
                "User unknown"
            }
            2 -> {
                if (participantsList[0].identity == credentialStorage.identity) {
                    participantsList[1].identity
                } else {
                    participantsList[0].identity
                }
            }
            3 -> {
                if (participantsList[0].identity == credentialStorage.identity) {
                    "${participantsList[1].identity}, ${participantsList[2].identity}"
                } else if (participantsList[1].identity == credentialStorage.identity) {
                    "${participantsList[0].identity}, ${participantsList[2].identity}"
                } else {
                    "${participantsList[0].identity}, ${participantsList[1].identity}"
                }

            }
            4 -> {
                if (participantsList[0].identity == credentialStorage.identity) {
                    "${participantsList[1].identity}, ${participantsList[2].identity}, ${participantsList[3].identity}"
                } else if (participantsList[1].identity == credentialStorage.identity) {
                    "${participantsList[0].identity}, ${participantsList[2].identity}, ${participantsList[3].identity}"
                } else if (participantsList[2].identity == credentialStorage.identity) {
                    "${participantsList[0].identity}, ${participantsList[1].identity}, ${participantsList[3].identity}"
                } else {
                    "${participantsList[0].identity}, ${participantsList[1].identity}, ${participantsList[2].identity}"
                }
            }
            else -> {
                "${participantsList[0].identity}, ${participantsList[1].identity}, ${participantsList[2].identity}"
        ...
        continuation.resume(identityName)
    }
...
suspend fun Conversation.getUnreadMessageCount(): Long? =
    suspendCancellableCoroutine { continuation ->
...
suspend fun ConversationsClient.getConversation(sidOrUniqueName: String): Conversation =
    suspendCoroutine { continuation ->
...
suspend fun ConversationsClient.checkConversationFromSidOrUniqueName(sidOrUniqueName: String): String =
    suspendCoroutine { continuation ->
        getConversation(sidOrUniqueName, object : CallbackListener<Conversation> {

            override fun onSuccess(result: Conversation) {
                continuation.resume(result.sid)
            }

            override fun onError(errorInfo: ErrorInfo) {
                continuation.resume(errorInfo.message)
            }
        })
    }

suspend fun Conversation.getLastMessages(count: Int): List<Message> =
    suspendCoroutine { continuation ->
...
suspend fun Conversation.getMessagesBefore(index: Long, count: Int): List<Message> =
    suspendCoroutine { continuation ->
...
suspend fun Conversation.sendMessage(block: Conversation.MessageBuilder.() -> Unit): Message =
    suspendCoroutine { continuation ->
...
suspend fun Conversation.advanceLastReadMessageIndex(index: Long): Long =
    suspendCoroutine { continuation ->
...
suspend fun Conversation.getMessageByIndex(index: Long): Message =
    suspendCoroutine { continuation ->
...
suspend fun Conversation.setAttributes(attributes: Attributes): Unit =
    suspendCoroutine { continuation ->
        setAttributes(attributes, object : StatusListener {
            override fun onSuccess() {
                continuation.resume(Unit)
...
suspend fun Conversation.setUniqueName(uniqueName: String): Unit =
    suspendCoroutine { continuation ->
        setUniqueName(uniqueName, object : StatusListener {
            override fun onSuccess() {
                continuation.resume(Unit)
            }
...
suspend fun Conversation.setFriendlyName(friendlyName: String): Unit =
    suspendCoroutine { continuation ->
...
inline fun createClientListener(
    ...
    ) = onConversationUpdated(conversation, reason)
    ...
    override fun onNewMessageNotification(
        ...
        messageSid: String,
        messageIndex: Long
    ) = onNewMessageNotification(conversationSid, messageSid, messageIndex)

    override fun onAddedToConversationNotification(conversationSid: String) =
        onAddedToConversationNotification(conversationSid)

    override fun onRemovedFromConversationNotification(conversationSid: String) =
        onRemovedFromConversationNotification(conversationSid)
